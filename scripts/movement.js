function updateMovement(entity, dT){ //entity to update and delta time
    ...
    } else if(entity.reverse){
       /* angleRad = angleRad + Math.PI;
        if (angleRad > Math.PI*2){
            angleRad = angleRad - (2* Math.PI);
        }
        var acceleration = [entity.accReverse * dT, angleRad];
        ...
        //limiting destination point and speed to maxSpeed
        destinationPoint[0] = limit(destinationPoint[0], entity.maxReverse, entity.maxReverse*(-1));
        destinationPoint[1] = limit(destinationPoint[1], entity.maxReverse, entity.maxReverse*(-1));
        if (oldSpeed <= entity.reverse){
            entity.speed[0] = limit(entity.speed[0], entity.maxReverse, entity.maxReverse*(-1));
        }
